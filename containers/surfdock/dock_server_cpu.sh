...
# Server-mode CPU docking script (Steps 3 + 5 + 6, CPU-only).
# Modified from dock_cpu.sh: accepts --tmpdir to redirect output paths, and --num_poses to
# configure the number of generated/saved docking poses. Also fixes the copy-paste bug
# in the original where Step 6 CSV construction used 'accelerate launch' instead of 'python'.
#
# Usage: dock_server_cpu.sh <Screen_lib_path> <docking_out_dir> [--tmpdir WORKDIR] [--num_poses N]
...
#   --num_poses      number of poses to generate and save (default: 3)
...
#------------------------------------------------------------------------------------------------#
#------------------------  Step5 : Start Sampling Ligand Conformers (CPU)  ----------------------#
#------------------------------------------------------------------------------------------------#
