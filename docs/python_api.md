...
`protonate_container` launches a container once and streams molecules through it via stdin/stdout. It takes an iterator of `(smiles, name)` tuples and yields protonated `(smiles, name)` tuples. Both Apptainer/Singularity SIF files and Docker images are supported (see [pre-built containers](installation.md#pre-build-containers)).
