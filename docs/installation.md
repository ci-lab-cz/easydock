...
Protonation tools can also be packaged as Apptainer/Singularity or Docker containers and launched via `--protonation /path/to/container.sif` (or a Docker image name). Make sure your OS has the container runtime installed first — see [Container Support](#container-support) for OS-specific setup.
